...
const Home = () => {
  ...
  // API周りの変数取得
  const { allPosts, getAllPosts, followList, requestUser, toggleOnePostLike, toggleOneUserFollow } = useContext(ApiContext);

  useEffect(() => {
    getAllPosts();
  }, [])

  const allPostsExceptMe = allPosts.filter((post) => {
